var sp_vs_base = ( function ( $, window, document ) {
	'use strict';
	...
	function setup_color_picker( form ) {
		...
		jQuery( '.sp-pickr .pickr-btn' ).each( function () {
			const pickr = Pickr.create( {
				el: '.sp-pickr .pickr-btn',
				theme: 'monolith',
				swatches: null,
				components: {
					preview: false,
			...
			pickr
				.on( 'init', ( instance ) => {
					const c = jQuery( instance._root.root )
						.parent()
	...
	function prepare_field_order_indexes( elm ) {
		$( elm + ' tbody tr' ).each( function ( index, el ) {
			$( 'input.f_order', el ).val(
				parseInt( $( el ).index( elm + ' tbody tr' ) )
			);
		} );
	}
	...
	function set_property_field_value( form, type, name, value, multiple ) {
		switch ( type ) {
			...
			case 'colorpicker':
				...
				form.find( 'span.' + name + '_preview' ).css(
					'background-color',
					value
				);
				break;
			...
			case 'radio':
				form.find( 'input[name=i_' + name + ']' ).val( value );
				form.find( $( '.' + value ) )
					.addClass( 'rad-selected' )
					.siblings( '.rad-selected' )
					.removeClass( 'rad-selected' );
				break;
...
function thwvsOpenFormTab( elm, tab_id, form_type ) {
	sp_vs_base.openFormTab( elm, tab_id, form_type );
}
...
jQuery( function ( $ ) {
	...
	$( '.product_attributes' ).on(
		'click',
		'button.sp_vs_add_new_attribute',
		function ( event ) {
			event.preventDefault();
			...
			var popup_outer = $( '.sp-attribte-dialog' );
			popup_outer.find( 'input[type=text]' ).val( '' );

			if ( popup_outer.hasClass( 'sp-attribte-dialog-image' ) ) {
				var remove_button = popup_outer.find(
					'.sp_vs_remove_image_button'
				);
				remove_button.hide();
			}
			...
			var $wrapper = $( this ).closest( '.woocommerce_attribute' ),
				...
				settings_div = $( '.sp_vs_settings_fields_form' );

			sp_vs_base.setupColorPicker( settings_div );
			var $popup_div = $( '.sp-attribte-dialog-' + type ),
				height = type == 'color' ? 395 : 250;

			if ( $popup_div.length > 0 ) {
				$popup_div.dialog( {
					dialogClass: 'wp-dialog sp-popup',
					title: 'Add new term',
					modal: true,
					autoOpen: false,
					width: 500,
				...
				$( '.ui-dialog.sp-popup' ).css( 'z-index', 99999999 );
			}
		}
	);
	...
	function save_new_term( $wrapper, $dialog, attribute ) {
		...
		new_attribute_name = $dialog
			.find( 'input[name = "attribute_name"]' )
			.val();
		term_spec[ 'product_' + attribute ] = $dialog
			.find( 'input[name = "attribute_type"]' )
			.val();
	...
	$( '#woocommerce-product-data' ).on(
		...
		function () {
			$( '.sp-variation-gallery-wrapper' ).each( function () {
				var $this = $( this );
				var $galleryImages = $this.find(
					'.sp-variation-gallery-images'
				);
				var $imageGalleryIds = $this.find(
					'.sp-variation-gallery-ids'
				);
				var galleryFrame;

				$this
					.find( '.sp-add-variation-gallery-image' )
								...
								if ( attachment.id ) {
									...
									$galleryImages.append(
										'<li class="image" data-attachment_id="' +
											...
											'"><img src="' +
											attachment_image +
											'"><a href="#" class="sp-remove-variation-gallery-image"><span class="xts-i-close"></span></a></li>'
									);

									$this.trigger(
										'sp_variation_gallery_image_added'
									);
								}
							} );
				...
				$( document ).on(
					'click',
					'.sp-remove-variation-gallery-image',
					function ( event ) {
						event.preventDefault();
						$( this ).parent().remove();
	...
	$( document ).on( 'change', '.sp-dual-color', function ( e ) {
		if ( $( this ).is( ':checked' ) ) {
			$( '.gbl-attr-color-2' ).fadeIn();
		} else {
			$( '.gbl-attr-color-2' ).fadeOut();
		}
	} );
	$( '.sp-dual-color' ).trigger( 'change' );
} );
